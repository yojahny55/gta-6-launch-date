...
- [ ] Test expired cookie handling (new cookie generation)
- [ ] Test cookie persistence across sessions
- [ ] Test database cleanup for analytics data
- [ ] Test prediction data retention (indefinite)

## Tasks / Subtasks

- [ ] Task 1: Implement 2-year cookie expiration (AC: Cookie lifecycle)
  - [ ] Update cookie creation in `public/js/app.js`
  - [ ] Set `expires: 730` (2 years in days) for gta6_user_id
  - [ ] Set `expires: 365` (1 year in days) for cookie_consent
  - [ ] Verify cookie attributes: `secure`, `sameSite: 'strict'`

- [ ] Task 2: Implement expired cookie detection (AC: Expiration handling)
  - [ ] Create `isValidCookie(cookieValue)` function
  - [ ] Check cookie exists before use
  - [ ] Generate new cookie_id if expired or missing
  - [ ] Log cookie regeneration events

- [ ] Task 3: Prevent cookie refresh on every visit (AC: Cookie refresh)
  - [ ] Do NOT re-set cookie on page load
  - [ ] Do NOT extend expiration on visits
  - [ ] Keep original creation timestamp fixed
  - [ ] Cookie expiration is absolute (2 years from first creation)

- [ ] Task 4: Implement analytics data cleanup (AC: Database cleanup - FR90)
  - [ ] Create database cleanup script or scheduled task
  - [ ] Identify analytics data older than 24 months
  - [ ] Delete old analytics records
  - [ ] Preserve prediction data (indefinite retention)
  - [ ] Log cleanup operations

- [ ] Task 5: Handle orphaned predictions (AC: Database cleanup)
  - [ ] Identify predictions with no recent cookie access
  - [ ] Keep orphaned predictions (user may return)
  - [ ] Do NOT delete based on inactivity
  - [ ] Prediction data retained indefinitely

- [ ] Task 6: Document cookie retention policy (AC: All)
  - [ ] Update Privacy Policy with 2-year cookie retention
  - [ ] Document in About page
  - [ ] Ensure GDPR compliance statements

- [ ] Task 7: Write automated tests (ADR-011 Testing Requirements)
  - [ ] Create `tests/cookie-expiration.test.ts`
  - [ ] Test cookie maxAge configuration
  - [ ] Test expired cookie handling
  - [ ] Test new cookie generation
  - [ ] Test analytics cleanup logic
